...
export class StreamAnalyzer extends EventEmitter {
  ...
  constructor(obsStream: OBSStream, config: Config) {
    ...
    // Set default analysis prompt from config
    this.analysisPrompt =
  ...
  async startAnalyzing(
    ...
    const captureFrequency =
      frequencyMs !== undefined
        ? frequencyMs
        : (this.config.visionIntervalSeconds || 30) * 1000;

    logger.info(
      `Starting stream analysis from source "${sourceName}" every ${captureFrequency}ms`
    );

    // Start the screenshot capture
    const result = await this.obsStream.startScreenshotCapture(
      sourceName,
      captureFrequency
    );
    ...
    try {
      ...
      if (this.recentScreenshots.length > this.maxRecentScreenshots) {
        ...
        try {
          ...
          logger.warn(
            `Failed to delete old screenshot: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
      ...
      logger.error(
        `Error processing screenshot: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async analyzeScreenshot(
    ...
    try {
        ...
        messages: [
          ...
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'You are viewing a livestream. What is happening in this image?',
              },
              {
                type: 'image_url',
                image_url: {
      ...
      return {
        ...
        analysis: {
          description:
            response.choices[0].message.content || 'No description provided',
          model: 'gpt-4o-mini',
  ...
  async analyzeNow(): Promise<StreamAnalysisResult | null> {
    ...
    try {
      ...
      const screenshotPath = await this.obsStream.captureOneScreenshot(
        this.captureSource
      );
      ...
      logger.error(
        `Failed to analyze now: ${error instanceof Error ? error.message : String(error)}`
      );
      return null;
    }
  }
